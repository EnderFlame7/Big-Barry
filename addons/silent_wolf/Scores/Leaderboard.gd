...
func _ready():
	hide()
	anim_player.set_current_animation("show")
	show()
	anim_player.play()
...
func add_item(player_name: String, score_value: String) -> void:
	...
	$TextureRect/Board/HighScores/ScoreItemContainer.add_child(item)


func add_no_scores_message() -> void:
	var item = $TextureRect/Board/MessageContainer/TextMessage
	item.text = "No scores yet!"
	$TextureRect/Board/MessageContainer.show()
	item.offset_top = 135
...
func hide_message() -> void:
	$TextureRect/Board/MessageContainer.hide()


func clear_leaderboard() -> void:
	var score_item_container = $TextureRect/Board/HighScores/ScoreItemContainer
